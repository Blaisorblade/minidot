...
Require Import Context AutosubstSsr.
...
(* Check (fun u1 u2 => u1 .[ u2/]). *)
(* Locate "_ .[ _ /]". *)

(** **** Syntactic typing *)

Require Import tactics.
Definition ctx := seq.seq type.
...
Proof.
  (* destruct A; cbn +[V]. *)
  (* elim: A => [x [ ]| A ? B | ]; try easy. *)
  (* move => _ /=[A'[s ->] ]. easy. *)
  (* move => ? _ /=[s ->]. *)

  (* destruct A; unfold V. intros [ x [] | | ]. *)
  (* - easy. *)
  (* - destruct H. *)

  (* Original *)
      (* by elim: A => [x[]|A _ B _/=[A'[s->]]|A _/=[s->]]. *)

  (* Better: we just need case analysis (case), not induction + throwing away
     induction hypotheses (elim:). *)
      by case: A => [x[]|A B /=[A'[s->]]|A /=[s->]].
Qed.
...
Restart.
  elim: A rho1 rho2 v => /=.
  - move => x rho1 rho2 v eqn [ev /eqn]. by intuition.
  - move => A ih1 B ih2 rho1 rho2 v eqn. move => [A' [s -> h]].
    exists A'. exists s => // u. move => /ih1/h[] X w. by (* move => X w; *) split; apply eqn.
    move: X w => w ev /ih2 ih. by eexists; eauto.
  -
    intros * ih * eqn [s -> h].
    do ? move => ?.
    (* move => A ih rho1 rho2 v eqn [s -> h]. *)
    exists s => // P B.
    case: (h P B) => u ev /ih va. eexists; eauto.
    apply: va => - [|X] //=.
    exact: eqn.
...
Proof.
  ...
  (* (* Set Printing All. *) *)
  (* Print VG. *)
  intro ht; induction ht; intros * l; ev; simpl.
  - info_eauto.
  - unfold L. eexists; try autosubst. do 2 eexists; try easy; intros.
    asimpl.
    eapply IHht => -[_//= | ]; eauto.
  - ecase (IHht1 delta); eauto.
    move => {IHht1} /= v ev1 [A' [u eq ih]].
    move => . subst.
    edestruct (IHht1 delta); edestruct (IHht2 delta); eauto.
    unfold L. cbn [V] in * |- ; ev; subst. edestruct H3; eauto.
    unfold L. eauto.
  - 
